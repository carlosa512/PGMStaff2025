...
### update_contracts.py behavior (team-safe + restructured salary aware)

`scripts/update_contracts.py` now applies active OverTheCap contracts with explicit match-safety rules:

- Uses **active contracts only** (`is_active == True`) from `reference/nflverse_contracts.parquet`
- Normalizes names and applies known aliases:
  - `micah parson` -> `micah parsons`
  - `riq woolen` -> `tariq woolen`
- Lookup key is `(normalized_name, resolved_team_id)` rather than name-only
- Match policy:
  ...
  - Team mismatches are skipped (not applied)
  - Ambiguous teamless names are skipped

Contract value extraction rules:

- `salary`/`eSalary`: average `base_salary` across remaining years from `cols` (fallback: `apy`)
- `guarantee`/`eGuarantee`: sum of remaining-year `guaranteed_salary` (fallback: top-level `guaranteed`)
- `length`/`eLength`: `max(1, year_signed + years - CURRENT_YEAR)` (metadata source of truth)

Audit report:

- CLI option: `--report-path` (default `reference/contract_update_report.csv`)
- CSV columns:
  - `roster_name`, `roster_team`, `normalized_name`, `alias_used`
  - `matched_contract_name`, `matched_contract_team`
  - `match_status`, `salary_source`, `reason`
- Statuses:
  - `matched_team`, `matched_name_only`, `skipped_no_match`, `skipped_team_mismatch`, `skipped_ambiguous`
- Review skipped mismatch/ambiguous rows before manual overrides.
...
| `scripts/update_contracts.py` | Apply active OTC contracts with team-safe matching, alias normalization, restructured salary handling, and CSV audit report |
...
| `reference/contract_update_report.csv` | Generated by `update_contracts.py`; audit trail for matches/skips/ambiguities (gitignored by default). |
