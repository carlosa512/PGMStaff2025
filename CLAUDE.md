...
### update_contracts.py behavior (cap-inflation hardened)

`scripts/update_contracts.py` applies active OverTheCap contracts with team-safe matching plus targeted release handling:

- Uses **active contracts only** (`is_active == True`) from `reference/nflverse_contracts.parquet`
- Normalizes names and applies aliases:
  - `micah parson` -> `micah parsons`
  - `riq woolen` -> `tariq woolen`
- Team-safe lookup key: `(normalized_name, resolved_team_id)`
- Match policy:
  - Teamed players (not `Free Agent` / `Rookie` / empty) require **team-confirmed** match
  - Teamless players allow name-only match only when unique
  - Team mismatches and ambiguous teamless names are skipped
...
Release and FA normalization:

- Optional release override input: `--release-overrides-path` (default `reference/release_overrides_2026.csv`)
- Override rows move players to `teamID="Free Agent"` and zero contract fields
- All Free Agents are normalized to `salary/eSalary=0`, `guarantee/eGuarantee=0`, `length/eLength=0`

Reporting:

- Player audit CSV: `--report-path` (default `reference/contract_update_report.csv`)
- Team cap CSV: `--team-cap-report-path` (default `reference/team_cap_report.csv`)
- Player report columns include:
  - `roster_name`, `roster_team`, `normalized_name`, `alias_used`
...
| `scripts/update_contracts.py` | Apply active OTC contracts with team-safe matching, remaining-years salary windowing, current-year bonus mapping, release overrides, and CSV reports |
...
| `reference/contract_update_report.csv` | Generated by `update_contracts.py`; player-level audit trail for matches/skips/releases (gitignored by default). |
| `reference/team_cap_report.csv` | Generated by `update_contracts.py`; team-level salary/bonus/cap-hit totals vs budget (gitignored by default). |
| `reference/release_overrides_2026.csv` | Maintained release override input (tracked); explicit cuts/releases to force FA with `0/0/0` contract fields. |
